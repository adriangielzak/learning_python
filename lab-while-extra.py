...
inThisTimeCapital = initialCapital
percent = 0.03
maxTimeYears = 10
year = 1

while year <= maxTimeYears:
    inThisTimeCapital += inThisTimeCapital * percent
    print("In {:2} you have {:.2f}".format(year, inThisTimeCapital))
    # print("In", year, "you have", initialCapital)
    year += 1

print("In {} years you've earned {:.2f}".format(year-1, (inThisTimeCapital - initialCapital)))
...
# -aby usunąć ostatnią cyfrę z pomocniczej zmiennej podziel ją cąłkowiicie ( operator // )  przez 10 i wynik zapisz
# w tej samej pomocniczej zmiennej
...
# Wskazówka 2:
# Skorzystaj z tego opisu, jeżeli nadal nie masz pomysłu na rozwiązanie. Ten opis, to jakby słowne opowiedzenie tego
# co robi program:
#
# Mamy liczbę 20730906 zapisną, a jakże inaczej w systemie DZIESIĘTNYM.
# Kiedy tą liczbę podzielę modulo (symbol działania %) przez 10 (dzielenie modulo zwraca resztę z dzielenia) to
# dostaniemy 6 - czyli ostatnią cyfrę.
# Potem ta ostatnia cyfra już mnie nie obchodzi. Więc dzielę bez reszty (symbol działania //) przez 10. W wyniku
# dostaję więc liczbę 2073090.
# I od początku:
...
# -pętlą przejdź przez listę słów i poleceniem IF badaj czy napis jest dłuższy od wordLength. Jeśli tak zwiększ
# liczbę longWords o 1, a jeśli nie to zwiększ shortWords

tekst = "United Space Alliance: This company provides major support to NASA for various projects, such as the space shuttle. " \
        "One of its projects is to create Workflow Automation System (WAS), an application designed to manage NASA and " \
        "other third-party projects. The setup uses a central Oracle database as a repository for information. Python " \
        "was chosen over languages such as Java and C++ because it provides dynamic typing and pseudo-code–like syntax " \
        "and it has an interpreter. The result is that the application is developed faster, and unit testing " \
        "each piece is easier."
...
print("Short Words = {}".format(shortWords))
print("Long Words = {}".format(longWords))
...
# Uwaga!
#
# na początku i na końcu listy powstają dwa zerowe '' elementy listy, więc trzeba jeszcze od wyniku odjąć 2,
# albo w kodzie sprawdzać czy element listy nie ma długości 0 (tak chyba jest poprawniej):
#
#     if len(listOfWords[i])>wordLength:
#         longWords+=1
